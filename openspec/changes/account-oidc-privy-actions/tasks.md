...
- [x] 7.7 Implement approve/deny handling. DB-backed persistence: `persistActionRequestDecision` in `connect/src/lib/db/account-actions.ts` performs a `pending`-gated `UPDATE ... RETURNING` so a second decision cannot overwrite the first. App Router endpoint wired: `POST /api/account/actions/[id]/decision` resolves `vana_user_id` only from `LoginEvidence` via `createPrivyLoginSessionAdapter` (never from request body/query), validates the resolved subject is an opaque Vana user id, 401s without evidence, 404s for unknown ids, 403s when the request is bound to a different user, 409s when the pending gate fails, and rejects non-`mock` approval before mutation. On approve it issues a single-use action code via `generateActionCode`, persists a mock result via `insertActionResult`, persists an `action.approved` consent event, and returns a redirect URL whose query is exactly `action_code` (and `state` only when state was originally set and re-supplied). On deny it persists `action.denied` and returns a redirect URL with no `action_code`. State binding: the decision body must re-supply raw state, which is verified against the persisted `state_hash` before any decision is persisted. Current route wiring composes decision/result/event writes across helpers; a later production-hardening slice should make multi-row mutation + event writes transactional. Tests in `connect/src/lib/auth/account-action-routes.test.ts` and `connect/src/app/api/account/actions/account-actions-routes.test.ts`.
...
- [x] 7.9 Implement action-code exchange with client binding and expiration. `consumeActionCode` in `connect/src/lib/db/account-actions.ts` performs hash-match, client-binding, expiry, and not-yet-consumed checks inside a single `UPDATE ... RETURNING`; concurrent and replayed exchanges produce exactly one success. DB-backed tests in `connect/src/lib/db/account-actions.test.ts` (skipped without `DATABASE_URL`) prove single-success-under-concurrency, wrong-client rejection, expiry rejection, and that no raw action code is stored. App Router endpoint wired: `POST /api/account/actions/exchange` validates `client_id` against the registry, calls `consumeActionCode`, and on success returns `action_request_id`, `result_mode`, `result_payload`, `result_reference`, and `expires_at`. Failure modes fold to stable OAuth-ish errors: `expired` → `expired_grant`, all of `not_found`/`client_mismatch`/`consumed` → `invalid_grant` (so a caller cannot probe for code existence). On successful exchange we persist an `action.exchanged` consent event (idempotency_key `${request.id}:exchanged`) when the originating request can be looked up; this is not yet atomic with consumption. The persisted `action_code_hash` is never returned to the OAuth client. Tests cover unknown client, missing code, success shape, exchanged-event persistence, and failure mapping.
